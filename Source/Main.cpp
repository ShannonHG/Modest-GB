...
void SetDefaultValuesInMemoryMap(MemoryMap& memoryMap);

void TestPrint(uint16_t address, uint8_t value)
{
	Logger::WriteInfo("Address: " + ConvertToHexString(address, 4) + " | " + "Value: " + std::to_string(value));
}
...
int main(int argc, char* argv[])
{
	...
	auto ppu = PPU(mainDisplay, memoryMap);
	...
	memoryMap.AssignDeviceToAddressRange(ioRegisters, 0xFF08, 0xFF39);
	...
	SetDefaultValuesInMemoryMap(memoryMap);
