...
namespace SHG
{
	void Test8BitBitReset(CPU& processor, Register8* targetRegister, uint8_t bitNumber)
	{
		...
		EXPECT_EQ(prevInstruction.instructionType, CPUInstructionType::BitReset);
	...
	void Test8BitBitResetAtAddress(CPU& processor, MockMemoryManagementUnit& mmu, Register16* addressRegister, uint8_t bitNumber)
	{
		...
		EXPECT_EQ(mmu.GetByte(address) & (1 << bitNumber), 0);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x80 });
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x81 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x82 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x83 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x84 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x85 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x86 });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 0);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x87 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x88 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x89 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8A });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8B });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8C });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8D });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8E });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 1);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x8F });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x90 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x91 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x92 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x93 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x94 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x95 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x96 });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 2);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x97 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x98 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x99 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9A });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9B });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9C });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9D });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9E });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 3);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0x9F });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA0 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA1 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA2 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA3 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA4 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA5 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA6 });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 4);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA7 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA8 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xA9 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAA });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAB });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAC });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAD });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAE });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 5);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xAF });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB0 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB1 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB2 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB3 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB4 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB5 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB6 });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 6);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB7 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB8 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xB9 });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBA });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBB });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBC });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBD });
		auto processor = CPU(mmu);
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBE });
		auto processor = CPU(mmu);

		Test8BitBitResetAtAddress(processor, mmu, processor.GetRegisterHL(), 7);
	}
	...
	{
		auto mmu = CreatePresetMemoryManagementUnit({ 0xCB, 0xBF });
		auto processor = CPU(mmu);
